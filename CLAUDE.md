...
- **Worktree launch.json — shell wrapper for cd:** `next dev` has no `--cwd` flag, so to run the dev server from a worktree directory use `/bin/sh` as `runtimeExecutable` with `["-c", "cd <worktree-path> && node /path/to/node_modules/.bin/next dev"]` as `runtimeArgs`. This is the correct pattern for any CLI that lacks a working-directory flag.
